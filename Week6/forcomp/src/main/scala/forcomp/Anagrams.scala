...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = sentenceOccurrences(List(w))

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    val validLetters = s.flatten.filter(_.isLetter).map(_.toLower)
    val occurrences = validLetters.groupBy(char => char).map{case (char, list) => (char, list.length)}
