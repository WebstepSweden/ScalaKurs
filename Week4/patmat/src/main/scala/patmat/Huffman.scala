...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] =
    ...
    else insertTree(trees.tail.tail, makeCodeTree(trees.head, trees.tail.head))

  def insertTree(trees: List[CodeTree], newTree: CodeTree): List[CodeTree] = {
    val s = trees.partition(x => weight(x) < weight(newTree))
    s._1  ::: List(newTree) ::: s._2
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table.find(_._1 == char) match {
    ...
    case _ => throw new Error("Unknown char " + char)
  }
   ...
   */
  def mergeCodeTables(left: CodeTable, right: CodeTable): CodeTable = left.map(x => (x._1, List(0) ::: x._2)) ::: right.map(x => (x._1, List(1) ::: x._2))
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def encode0(text: List[Char], acum: List[Bit]): List[Bit] = {
      if (text.isEmpty) acum
      else encode0(text.tail, acum ::: codeBits(table)(text.head))
    } 
