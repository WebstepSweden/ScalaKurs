...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    if (initial.isEmpty) Stream.empty
      ...
      val (current, moves) = initial.head
      val neighbors = newNeighborsOnly(neighborsWithHistory(current, moves), explored)
      initial.head #:: from(initial.tail ++ neighbors, explored + current)
    }
  }
